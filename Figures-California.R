...
path = "/data/doutorado-leonildo/data/O3-2019/California/"  # (for example)
setwd(path)
...
files = list()
files = lappend(files, c("./50km/global_autocorrelation_US_pollution.csv","./50km/local_autocorrelation_US_pollution.csv"))
files = lappend(files, c("./60km/global_autocorrelation_US_pollution.csv","./60km/local_autocorrelation_US_pollution.csv"))
files = lappend(files, c("./70km/global_autocorrelation_US_pollution.csv","./70km/local_autocorrelation_US_pollution.csv"))
files = lappend(files, c("./80km/global_autocorrelation_US_pollution.csv","./80km/local_autocorrelation_US_pollution.csv"))
files = lappend(files, c("./90km/global_autocorrelation_US_pollution.csv","./90km/local_autocorrelation_US_pollution.csv"))
...
g = g + scale_y_continuous(breaks = seq(-.05, .5, .02), 
                           sec.axis = sec_axis(~ ., breaks = seq(-.05, .5, .02), name = local.morans.i.text2))
...
#g = g + guides(linetype = guide_legend(nrow = 1), shape = guide_legend(nrow = 0))
...
for(i in seq(1:1000)) {
  file = paste0("/data/doutorado-leonildo/data/O3-2019/California/pareto_optimality/pareto_front_US_pollution_", i, ".csv")
  df = as.data.frame(fread(file, sep = ",", dec = "."))
  df['.level'] = paste0("#",df$`low(local.moran.i.cvar) * high(global.moran.i)`)
  df['label'] = paste0(strsplit(df$areal.unit,"km"), " km")
  df['file'] = file
...
files = list()
files = lappend(files, c("./50km/hex-grid.shp","./50km/final-file.csv"))
files = lappend(files, c("./60km/hex-grid.shp","./60km/final-file.csv"))
files = lappend(files, c("./70km/hex-grid.shp","./70km/final-file.csv"))
files = lappend(files, c("./80km/hex-grid.shp","./80km/final-file.csv"))
files = lappend(files, c("./90km/hex-grid.shp","./90km/final-file.csv"))
files = lappend(files, c("./100km/hex-grid.shp","./100km/final-file.csv"))
